...

{
  ...
  "optimizations": ["string — what was applied or adjusted"]
}
`;
...
const AUTO_PIN_SYSTEM_PROMPT = `You are an experienced documentary film editor and visual director.
You are given a scene description and three image prompts (Wide, Medium, Close-up).
These images will be generated by AI and then animated.

Select the single prompt best suited for this scene based on:

1. ANIMATION SUITABILITY (most important):
   - Prefer: simple composition, single dominant subject, clear depth layers
   - Avoid: complex crowds, highly detailed patterns that compete with motion
   - Prefer: implied directional movement, breathing room around subject

2. NARRATIVE IMPACT:
   - Does it capture the emotional or informational core of this scene?
   - Would it work as a standalone image without narration?

3. PRODUCTION ACCURACY:
   - Does it honour the era, culture, and character anchors?
   - Is the stated light and environment coherent?

SHOT SELECTION PRIORITY:
  → If scene contains: army, crowd, landscape, city, sea, geography → prefer WIDE SHOT
  → If scene contains: encounter, dialogue, action, movement, journey → prefer MEDIUM SHOT
  → If scene contains: hand detail, object, face (single person), texture → prefer CLOSE-UP
  → Default if uncertain: MEDIUM SHOT (never default to CLOSE-UP)

NEVER select CLOSE-UP if:
  - Scene has 3+ people / soldiers
  - Scene describes a landscape, city, or large-scale event
  - Visual note contains "crowd", "army", "city", "landscape"

Return ONLY this JSON, no other text:
{ "selectedIndex": 0, "reason": "One sentence Turkish explanation" }
selectedIndex: 0 = first prompt, 1 = second prompt, 2 = third prompt`;
...
export async function autoSelectBestPrompt(
  ...
  const userMessage = `SCENE TEXT:\n${sceneText}\n\nVISUAL NOTE: "${visualNote}"\n\nPROMPTS:\n${promptList}\n\nWhich prompt best serves this scene?`;

  try {
    const raw = await aiProvider.generateContent(userMessage, AUTO_PIN_SYSTEM_PROMPT, {
      operationType: 'auto_pin_selection',
    });

    const result = tryParseJSON(raw) as { selectedIndex?: number; reason?: string };
    const idx = Number(result?.selectedIndex);
    if (isNaN(idx) || idx < 0 || idx >= prompts.length) {
      return { selectedIndex: 0, reason: result?.reason ?? '' };
    }
    return { selectedIndex: idx, reason: result?.reason ?? '' };
  } catch (err) {
    console.warn('[promptEngine] autoSelectBestPrompt failed, defaulting to 0:', err);
