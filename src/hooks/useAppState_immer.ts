...
function reducerCore(state: AppState, action: InternalAction): AppState {
  switch (action.type) {
    ...
    case 'SET_RENDER_MODE':
      draft.renderMode = action.payload;
      break;
    case 'SET_MAIN_TEXT':
      Object.assign(draft, { mainText: action.payload.text, documentText: action.payload.text, mainFileName: action.payload.fileName });
      break;
    case 'SET_EXTRACTED_ENTITIES':
      draft.extractedEntities = action.payload;
      break;
    case 'ADD_EXTRACTED_ENTITY':
      Object.assign(draft, { extractedEntities: [...state.extractedEntities, action.payload] });
      break;
    case 'UPDATE_ENTITY': {
    ...
    case 'ADD_SCENE':
      Object.assign(draft, { scenes: [...state.scenes, action.payload] });
      break;
    case 'REMOVE_SCENE': {
    ...
    case 'UPDATE_SCENE':
      Object.assign(draft, { scenes: state.scenes.map(s => s.id === action.payload.id ? action.payload : s) });
      break;
    case 'REORDER_SCENES':
      draft.scenes = action.payload;
      break;
    case 'SET_ACTIVE_SCENE':
      draft.activeSceneId = action.payload;
      break;
    case 'SET_SELECTION_MODE':
      draft.selectionMode = action.payload;
      break;
    case 'SET_API_KEYS':
      localStorage.setItem('gemini_api_keys', JSON.stringify(action.payload));
      Object.assign(draft, { apiKeys: action.payload, currentKeyIndex: 0 });
      break;
    case 'SET_IMAGE_API_KEYS':
      localStorage.setItem('gemini_image_api_keys', JSON.stringify(action.payload));
      draft.imageApiKeys = action.payload;
      break;
    case 'SET_CURRENT_KEY_INDEX':
      draft.currentKeyIndex = action.payload;
      break;
    case 'ROTATE_API_KEY':
      if (state.apiKeys.length <= 1) return state;
      Object.assign(draft, { currentKeyIndex: (state.currentKeyIndex + 1) % state.apiKeys.length });
      break;
    case 'SET_SETTINGS': {
    ...
    case 'SET_REFERENCES':
      ...
      break;
    case 'ADD_REFERENCE':
      Object.assign(draft, { references: [...state.references, action.payload] });
      break;
    case 'REMOVE_REFERENCE':
      Object.assign(draft, { references: state.references.filter(r => r.id !== action.payload) });
      break;
    ...
    case 'ADD_CONSISTENCY_GROUP':
      Object.assign(draft, { consistencyGroups: [...state.consistencyGroups, action.payload] });
      break;
    ...
    case 'ADD_SCENE_TO_GROUP': {
      ...
      Object.assign(draft, { consistencyGroups: updatedGroups, scenes: updatedScenes });
      break;
    }
    case 'REMOVE_SCENE_FROM_GROUP': {
      ...
      Object.assign(draft, { consistencyGroups: groups, scenes });
      break;
    }
    // Two-stage AI workflow
    case 'START_ANALYSIS':
      Object.assign(draft, { isAnalyzing: true });
      break;
    ...
    case 'SET_TIME_CONTEXTS':
      draft.timeContexts = action.payload;
      break;
    ...
    case 'SET_MASTER_PROMPT':
      draft.masterPrompt = action.payload;
      break;
    case 'SET_EPISODE_PROMPT':
      draft.episodePrompt = action.payload;
      break;
    case 'SET_EPISODE_PROMPT_TR':
      draft.episodePromptTr = action.payload;
      break;
    case 'ADD_EPISODE_STYLE_VERSION':
      Object.assign(draft, { episodeStyleHistory: [action.payload, ...state.episodeStyleHistory].slice(0, 50) });
      break;
    case 'SET_EPISODE_STYLE_HISTORY':
      draft.episodeStyleHistory = action.payload;
      break;
    case 'SET_ANALYZING':
      draft.isAnalyzing = action.payload;
      break;
    case 'SET_DOCUMENT_TEXT':
      Object.assign(draft, { documentText: action.payload, mainText: action.payload });
      break;
    case 'SET_SCENES':
      draft.sceneCards = action.payload;
      break;
    case 'SET_CHARACTERS':
    ...
    case 'ADD_TIME_CONTEXT':
      Object.assign(draft, { timeContexts: [...state.timeContexts, action.payload] });
      break;
