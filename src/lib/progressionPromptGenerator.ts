...
const PROGRESSION_PROMPT_SYSTEM = `You are an elite cinematic prompt engineer for AI image generation tools (Midjourney, Runway, Flow AI).
...
PROGRESSION NARRATIVE PROTOCOL:
You are generating a series of N independent prompts for a PROGRESSION NARRATIVE.
...
IF strategy = "progressive_elevation":
  ...
  Apply to: Cities growing, Forests viewed from sky, Mountains rising
...
IF strategy = "circular_orbit":
  ...
  Apply to: Lakes, Glaciers, Planets viewed from orbit
...
IF strategy = "zoom_out":
  ...
  Apply to: Glacier macro detail → landscape
...
IF strategy = "fixed_with_change":
  ...
  Apply to: Moon phases, Day/night cycles
...
const DEFAULT_ANALYSIS: PromptAnalysis = {
  ...
  productionNotes: ['🎬 Progression narrative: multi-stage progression generation'],
};

// ─── Main Export ──────────────────────────────────────────────────────────────

/**
 * Generates image prompts for each stage of a progression narrative.
 * Creates N prompts — one per stage — each enforcing the narrative's camera
 * strategy and anchor visibility rules.
 */
export async function generateProgressionPrompts(
  ...
  const { stages, anchor, cameraProgression, promptGenerationStrategy } = progression;
  ...
  entityHeader += `=== PROGRESSION ANCHOR (CRITICAL — APPLIES TO ALL ${stageCount} STAGES) ===\n`;
  entityHeader += `Anchor: ${anchor.name}\n`;
  entityHeader += `Role: ${anchor.role}\n`;
  entityHeader += `Description: ${anchor.description}\n`;
  entityHeader += `Symbolism: ${anchor.symbolism}\n`;
  entityHeader += `Visibility requirement: ${anchor.visibility}\n`;
  entityHeader += `⚠️ The anchor (${anchor.name}) MUST appear visibly in EVERY stage prompt.\n`;
  if (anchor.evolutionAcrossStages && anchor.evolutionAcrossStages.length > 0) {
    entityHeader += `Anchor evolution:\n`;
    anchor.evolutionAcrossStages.forEach((evo, i) => {
  ...
  const stagesDescription = stages.map((s, idx) => {
    ...
    const evo = anchor.evolutionAcrossStages?.[idx];
  ...
  userMessage += `SUBJECT: ${progression.subject}\n`;
  userMessage += `PROGRESSION TYPE: ${progression.type}\n`;
  ...
  userMessage += `🎬 PROGRESSION NARRATIVE — ${stageCount} STAGES\n`;
  ...
  userMessage += `- Anchor (${anchor.name}) must be visible in ALL ${stageCount} stages\n`;
  ...
  try {
    const rawContent = await aiProvider.generateContent(
      userMessage,
      PROGRESSION_PROMPT_SYSTEM,
      { operationType: 'progression_narrative_generation' }
    );
    parsed = tryParse(rawContent);
  } catch (firstErr) {
    console.error('[progressionPromptGenerator] First attempt failed, retrying...', firstErr);
    onRetry?.();
    const retryMessage = userMessage + '\n\nIMPORTANT: Return ONLY valid JSON. No markdown, no explanation, no code fences.';
    const retryContent = await aiProvider.generateContent(
      retryMessage,
      PROGRESSION_PROMPT_SYSTEM,
      { operationType: 'progression_narrative_generation_retry' }
    );
    try {
      parsed = tryParse(retryContent);
    } catch (secondErr) {
      console.error('[progressionPromptGenerator] Retry failed.', secondErr);
      throw new Error('Invalid JSON in progression narrative prompt response (after retry)');
    }
  }

  if (!parsed.stages || !Array.isArray(parsed.stages)) {
    throw new Error('Invalid progression narrative prompt response format: missing stages array');
  }

  // ── Build result stages ────────────────────────────────────────────────────

  const resultStages: ProgressionStage[] = parsed.stages.map((p, idx) => {
