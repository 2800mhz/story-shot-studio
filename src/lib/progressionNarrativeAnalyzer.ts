...
const PROGRESSION_NARRATIVE_SYSTEM_PROMPT = `You are an expert in universal progression narrative analysis for documentary film production.
Your task is to detect whether a scene text describes a TRANSFORMATION OVER TIME that constitutes a progression narrative.

PROGRESSION NARRATIVE DETECTION:

A progression narrative is present when the text describes:
...
- biological_metamorphosis: Biological or social transformation (forest regrowth, civilization collapse)
- civilization_arc: Rise and fall of societies
- seasonal_cycle: Weather and plant cycles
...
- If text describes 3 or more phases → LIKELY narrative
- If text has only 1-2 events without clear progression → NOT a narrative (return isProgressionNarrative: false)
...
{
  "isProgressionNarrative": true,
  "subject": "City/Lake/Glacier/Moon etc",
  "progressionType": "urban_growth",
...
If NOT a progression narrative:
{ "isProgressionNarrative": false }`;

// ─── Main Export ──────────────────────────────────────────────────────────────

/**
 * Analyzes scene text to detect if it describes a universal progression narrative.
 * Returns a full ProgressionNarrative or null if not detected.
 */
export async function detectProgressionNarrative(
  sceneText: string,
  sceneVisualNote?: string
): Promise<ProgressionNarrative | null> {
  const userMessage = `SCENE TEXT:\n${sceneText}${sceneVisualNote ? `\n\nVISUAL NOTE: ${sceneVisualNote}` : ''}\n\nAnalyze if this is a progression narrative. Return JSON only.`;

  try {
    const rawContent = await aiProvider.generateContent(
      userMessage,
      PROGRESSION_NARRATIVE_SYSTEM_PROMPT,
      { operationType: 'progression_narrative_analysis' }
    );
    ...
    const parsed = JSON.parse(cleaned) as {
      isProgressionNarrative: boolean;
      subject?: string;
    ...
    if (!parsed.isProgressionNarrative) {
      return null;
    }

    // Build stages
    const rawStages = parsed.stages || [];
    const stages: ProgressionStage[] = rawStages.map(s => ({
      number: s.number,
      label: s.label,
      ...
      } satisfies StageEnvironment,
      cameraDirection: {
        height: (s.cameraDirection?.height as ProgressionStage['cameraDirection']['height']) || 'medium',
    ...
    const anchor: ProgressionAnchor = {
      ...
      role: (rawAnchor?.role as ProgressionAnchor['role']) || 'constant_reference',
    ...
    const cameraStrategy = (parsed.cameraStrategy as CameraStrategy) || 'fixed_with_change';
    const progressionType = (parsed.progressionType as ProgressionType) || 'custom';

    // Derive consistency strategy from camera strategy
    const consistencyMap: Record<CameraStrategy, ProgressionNarrative['promptGenerationStrategy']['consistency']> = {
    ...
    const progression: ProgressionNarrative = {
      ...
      promptGenerationStrategy: {
        ...
        anchorTreatment: (parsed.anchorTreatment as ProgressionNarrative['promptGenerationStrategy']['anchorTreatment']) || 'always_centered',
    ...
    console.warn('[progressionNarrativeAnalyzer] Failed to analyze:', err);
    return null;
  }
}
 ...
 */
export function mightBeProgressionNarrative(text: string, visualNote?: string): boolean {
  ...
  // Environmental/progression transformation keywords
