...
const HEIGHT_LABELS: Record<ArchitecturalNarrativeStage['cameraDirection']['height'], string> = {
  ground: '🏕️ Ground',
  low: '🏔️ Low',
  medium: '🏙️ Medium',
  high: '🗼 High',
...
interface TimelapsePromptBlockProps {
  stage: ArchitecturalNarrativeStage;
  narrativeId: string;
  isActive?: boolean;
...
/**
 * Displays a single stage prompt within an architectural narrative progression.
 * Shows stage metadata, camera height, environmental state, and the generated prompt.
 */
export function TimelapsePromptBlock({
  stage,
  narrativeId,
  isActive,
  onRevise,
  onClick,
}: TimelapsePromptBlockProps) {
