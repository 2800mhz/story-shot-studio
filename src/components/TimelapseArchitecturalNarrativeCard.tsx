...
import { TimelapsePromptBlock } from './TimelapsePromptBlock';
import type { ArchitecturalNarrativeProgression } from '@/types';
...
interface TimelapseArchitecturalNarrativeCardProps {
...
/**
 * Visually distinct card for displaying an architectural narrative progression.
 * Shows narrative metadata (type, anchor, camera strategy), a stage timeline,
 * and the generated prompts for each stage.
 *
 * Appearance is intentionally different from regular SceneCard to make it clear
 * this is a separate, system-understood narrative type.
 */
export function TimelapseArchitecturalNarrativeCard({
  progression,
  isActive,
  onSelect,
  onGenerate,
  onDelete,
  onRevise,
}: TimelapseArchitecturalNarrativeCardProps) {
  const [collapsed, setCollapsed] = useState(false);
  const [activeStage, setActiveStage] = useState<number | undefined>(undefined);

  const { stages, progressionAnchor, cameraProgression, narrativeType, narrativeSubject, transformationDriver, status } = progression;
        ...
        <div className="flex items-center gap-2 flex-1 min-w-0">
          ...
          <div className="flex flex-col min-w-0">
            ...
            <span className="text-[10px] text-muted-foreground truncate">
              {NARRATIVE_TYPE_LABELS[narrativeType] || narrativeType}
            </span>
          </div>
        ...
        <div className="p-3" onClick={e => e.stopPropagation()}>
          ...
          <div className="mb-3 rounded-md border border-amber-500/20 bg-amber-500/5 p-2.5 text-[11px] space-y-1.5">
            ...
            <div className="flex items-start gap-2">
              ...
              <div>
                <span className="font-semibold text-amber-300">{progressionAnchor.name}</span>
                <span className="text-muted-foreground ml-1">({progressionAnchor.role})</span>
                {progressionAnchor.symbolism && (
                  <p className="text-muted-foreground/70 mt-0.5 text-[10px]">{progressionAnchor.symbolism}</p>
                )}
              </div>
            </div>
